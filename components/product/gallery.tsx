...
import { getYoutubeEmbed, getYoutubePreviewImage } from 'lib/youtube-embed-utils';
import Image from 'next/image';
import Link from 'next/link';
import { usePathname, useSearchParams } from 'next/navigation';

export function Gallery({ media }: { media: { type: 'image' | 'video'; src: string; altText: string }[] }) {
            ...
            return (
              <li key={media.src} className="h-20 w-20">
                ...
                >
                  <GridTileImage
                    alt={media.altText}
                    src={media.type == 'image' ? media.src : getYoutubePreviewImage(media.src).sm}
                    width={80}
                    height={80}
                    active={isActive}
                  />
